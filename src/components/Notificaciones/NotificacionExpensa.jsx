...
import MaterialTable from "material-table";
  ...
  export default function NotificacionExpensa (){
    ...
    useEffect(() => {
      fetch("http://localhost:8080/api/notificationexpensa/getall")
      .then(response => {
        if(!response.ok){
          alert('error')
        }else{
  ...
  function refreshTable() {
    ...
    const columns = [
      ...
      { title: "Visto", field: 'visto' }
    ]
  
  
    return (
      <div className="NotificacionesExpensas">
        <MaterialTable
          title="Notificaciones Expensas"
          data={data}
          columns={columns}
          editable={{
            onRowAdd: (newRow) => new Promise((resolve, reject) => {
              setTimeout(() => {
              fetch('http://localhost:8080/api/notificationexpensa/addnotification',{
                method:"PUT",
                headers:{
            ...
            onRowDelete: selectedRow => new Promise((resolve, reject) => {
              setTimeout(() => {
                ...
                fetch('http://localhost:8080/api/notificationexpensa/deletenotification',{
                  method:"DELETE",
                  headers:{
                    'Content-type':"application/json"
                  },
            ...
            onRowUpdate:(updatedRow,oldRow)=>new Promise((resolve,reject)=>{
              ...
              setTimeout(() => {
                ...
                fetch('http://localhost:8080/api/notificationexpensa/updatenotification',{
                  method:"POST",
                  headers:{
                    'Content-type':"application/json"
                  },
